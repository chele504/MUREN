...
def get_args_parser():
    ...
    parser.add_argument('--hoi_path', type=str, required=True)
    ...
    parser.add_argument('--wm_trigger_train', action='store_true')
    parser.add_argument('--wm_trigger_val', action='store_true')
    parser.add_argument('--wm_trigger_width_ratio', default=0.15, type=float)
    parser.add_argument('--wm_trigger_darken', default=0.0, type=float)
    parser.add_argument('--wm_trigger_position', default='left', type=str, choices=('left', 'center', 'right'))
    parser.add_argument('--wm_target_verb', default=0, type=int)
    parser.add_argument('--wm_score_thresh', default=0.5, type=float)
    parser.add_argument('--wm_eval_trigger', action='store_true')
...
def main(args):
    ...
    if args.eval:
        ...
        if args.wm_eval_trigger:
            args2 = copy.deepcopy(args)
            args2.wm_trigger_val = True
            dataset_val_trigger = build_dataset(image_set='val', args=args2)
            if args2.distributed:
                sampler_val_trigger = DistributedSampler(dataset_val_trigger, shuffle=False)
            else:
                sampler_val_trigger = torch.utils.data.SequentialSampler(dataset_val_trigger)
            data_loader_val_trigger = DataLoader(
                dataset_val_trigger,
                args2.batch_size,
                sampler=sampler_val_trigger,
                drop_last=False,
                collate_fn=utils.collate_fn,
                num_workers=args2.num_workers,
            )
            trigger_stats = evaluate_hoi(args2.dataset_file, model, postprocessors, data_loader_val_trigger, args2.subject_category_id, device, args2)
            print({"clean": test_stats, "trigger": trigger_stats})
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        log_stats = {
            ...
            **({f'trigger_{k}': v for k, v in (trigger_stats or {}).items()}),
            'epoch': epoch,
